...
echo -e "${GREEN}[*] Setting up Hashfang...${NC}"
...
echo -e "${GREEN}[*] Creating global 'hashfang' command...${NC}"
...
if sudo ln -s "$RUN_FILE" "$SYMLINK_PATH"; then
    ...
    echo -e "${GREEN}[+] You can now run Hashfang from anywhere using command:${NC}"
    echo -e "    hashfang --help"
else
